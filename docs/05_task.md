# Task 05: Apple 風アクセシブルフレットボードコンポーネント

## 概要

Apple HIG 準拠の完全アクセシブルな SVG フレットボード描画コンポーネントを作成

## 実行内容

1. WCAG 2.1 AAA 準拠の SVG フレットボード実装
2. 完全キーボード操作対応
3. スクリーンリーダー対応
4. Apple 風視覚デザインの適用
...
export const AccessibleFretboard: React.FC<AccessibleFretboardProps> = ({
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, string: number, fret: number) => {
      switch (e.key) {
        case 'Enter':
        case ' ':
          e.preventDefault();
...
- **コントラスト比**: 7:1 以上（blue-500 on white）
- **フォーカス表示**: 明確な 2px リング表示
- **キーボード操作**: 完全キーボードナビゲーション
- **スクリーンリーダー**: 適切な aria-label と role 設定
...
- [ ] フォーカス表示が明確に見える
- [ ] コントラスト比が WCAG 基準を満たす
- [ ] 44px 最小タッチターゲットが確保される
- [ ] 高コントラストモードで正常表示される

## 成果物

- 完全アクセシブルなフレットボードコンポーネント
- Apple 風洗練されたビジュアルデザイン
- WCAG 2.1 AAA 準拠のアクセシビリティ
- 包括的なキーボード操作対応
