# Task 05: Apple風アクセシブルフレットボードコンポーネント

## 概要
Apple HIG準拠の完全アクセシブルなSVGフレットボード描画コンポーネントを作成

## 実行内容
1. WCAG 2.1 AAA準拠のSVGフレットボード実装
2. 完全キーボード操作対応
3. スクリーンリーダー対応
4. Apple風視覚デザインの適用

## 作成ファイル
`src/components/fretboard/AccessibleFretboard.tsx`
...
export const AccessibleFretboard: React.FC<AccessibleFretboardProps> = ({
  ...
  const handleKeyDown = useCallback((
    ...
    switch (e.key) {
      ...
      case ' ':
        ...
        // 音声フィードバック
        manageFocus.announce(`${string + 1}弦 ${fret}フレットを再生しました`);
        break;
        
      case 'ArrowUp':
        e.preventDefault();
        if (string > 0) {
          setFocusedPosition({ string: string - 1, fret });
        }
        break;
      ...
      case 'ArrowLeft':
        ...
        if (fret > fretRange.start) {
          setFocusedPosition({ string, fret: fret - 1 });
        }
        break;
      ...
      case 'Home':
        e.preventDefault();
        setFocusedPosition({ string, fret: fretRange.start });
        break;
        
      case 'End':
        e.preventDefault();
        setFocusedPosition({ string, fret: fretRange.end });
        break;
    }
  }, [onStringPlay, fretRange]);
  ...
  const getStringPosition = (stringIndex: number) => {
    ...
    if (isHorizontal) {
      return {
        x1: 60,
        y1: 40 + (stringIndex * 40),
        x2: width - 60,
        y2: 40 + (stringIndex * 40)
      };
    } else {
      return {
  ...
  const getFretPosition = (fretIndex: number) => {
    ...
    const fretSpacing = isHorizontal ? 
      (width - 120) / (fretRange.end - fretRange.start) : 
      (height - 120) / (fretRange.end - fretRange.start);
    
    if (isHorizontal) {
      const x = 60 + (fretIndex * fretSpacing);
      return { x1: x, y1: 20, x2: x, y2: height - 20 };
    } else {
  ...
  const getChordDotPosition = (stringIndex: number, fret: number) => {
    ...
    const fretSpacing = orientation === 'horizontal' ? 
      (getSVGDimensions().width - 120) / (fretRange.end - fretRange.start) :
      (getSVGDimensions().height - 120) / (fretRange.end - fretRange.start);
    
    if (orientation === 'horizontal') {
      return {
        cx: 60 + (fret * fretSpacing) - (fretSpacing / 2),
        cy: stringPos.y1
      };
    } else {
      return {
        cx: stringPos.x1,
        cy: 60 + (fret * fretSpacing) - (fretSpacing / 2)
      };
    }
  };
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600">
          難易度: {chordPattern.difficulty} | 
          矢印キーで移動、Enter/Spaceで再生
        </p>
      </div>
      ...
      >
        ...
        {Array.from({ length: fretRange.end - fretRange.start + 1 }, (_, i) => {
          ...
          const fretPos = getFretPosition(fretIndex);
        ...
        {[3, 5, 7, 9, 12].filter(fret => fret >= fretRange.start && fret <= fretRange.end).map(fret => {
        ...
        {chordPattern.frets.map((fret, stringIndex) => {
          if (fret === null || fret < fretRange.start || fret > fretRange.end) return null;
          
          const pos = getChordDotPosition(stringIndex, fret);
          const isInteractive = !!onStringPlay;
          const isFocused = focusedPosition?.string === stringIndex && 
          ...
          return (
            <g key={`chord-dot-${stringIndex}-${fret}`}>
              ...
              <circle
                ...
                onKeyDown={isInteractive ? (e) => handleKeyDown(e, stringIndex, fret) : undefined}
                onClick={isInteractive ? () => onStringPlay(stringIndex, fret) : undefined}
                onFocus={() => setFocusedPosition({ string: stringIndex, fret })}
              />
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-gray-600">
          ...
          <div>
            <span className="font-medium">難易度:</span> {chordPattern.difficulty}
          </div>
          <div>
        ...
        {/* 押弦位置の詳細情報 */}
        <div className="mt-3">
          <div className="font-medium text-gray-900 text-sm mb-1">押弦位置:</div>
...
### WCAG 2.1 AAA準拠
- **コントラスト比**: 7:1以上（blue-500 on white）
- **フォーカス表示**: 明確な2pxリング表示
- **キーボード操作**: 完全キーボードナビゲーション
- **スクリーンリーダー**: 適切なaria-labelとrole設定

### キーボード操作
- **矢印キー**: フレットボード上の移動
- **Enter/Space**: 音声再生
- **Home/End**: 各弦の開始/終了フレットに移動
- **Tab**: 次の操作可能要素に移動

### 視覚的配慮
- **高コントラストモード**: 自動検出・対応
- **prefers-reduced-motion**: アニメーション制御
- **カラーアクセシビリティ**: 色のみに依存しない情報提示
...
- [ ] フォーカス表示が明確に見える
- [ ] コントラスト比がWCAG基準を満たす
- [ ] 44px最小タッチターゲットが確保される
- [ ] 高コントラストモードで正常表示される

## 成果物
- 完全アクセシブルなフレットボードコンポーネント
- Apple風洗練されたビジュアルデザイン
- WCAG 2.1 AAA準拠のアクセシビリティ
- 包括的なキーボード操作対応
...
## 次のステップ
Task 06: レスポンシブフレットボードコンポーネントへ進む